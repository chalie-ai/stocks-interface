 ...
 * ```
 */
export function handleAlertSet(
  params: AlertSetParams,
  state: ToolState,
  ...
  if (watchlistItem === undefined) {
    return buildErrorResult(
      `${symbol} is not in your watchlist. Add it first before setting a price alert.`,
      state,
    );
  }

  // ── Create the alert ─────────────────────────────────────────────────────
  const updatedState = createAlert(
    state,
    symbol,
    params.targetPrice,
    params.direction,
 ...
 * ```
 */
export function handleAlertList(
...
export function handleAlertDelete(
  ...
  if (existingAlert === undefined) {
    return buildErrorResult(
      `No alert with ID "${alertId}" was found. Use the alert list to find the correct ID.`,
      state,
    );
  }
  ...
  const text =
    `Alert deleted: ${existingAlert.symbol} ${directionWord} ${targetFormatted} ` +
 ...
 * @returns An inline-CSS HTML fragment safe for rendering in the Chalie UI.
 */
function buildAlertDeleteHtml(alert: PriceAlert): string {
