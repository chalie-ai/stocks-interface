 ...
 */
function todayET(utcDate = new Date()) {
 ...
 */
function classifyMarketState(isOpen, nowET) {
    if (isOpen)
        return "open";
...
function pruneDedup(state) {
    ...
    state.dedupHistory = state.dedupHistory.filter((entry) => entry.firedAt >= cutoff);
}
/**
...
function isDuplicate(state, key) {
    const cutoff = Date.now() - DEDUP_TTL_MS;
    return state.dedupHistory.some((entry) => entry.key === key && entry.firedAt >= cutoff);
}
/**
 ...
 */
function recordDedup(state, key) {
 ...
 */
function getCachedMetrics(client, symbol) {
 ...
 */
function evaluateStock(item, quote, state, client, onSignal) {
    ...
    if (absChange > notableThresholdStock * 2.5) {
        ...
        if (!isDuplicate(state, key)) {
            ...
            onSignal({
                ...
                name: quote.name,
                energy: 0.65,
                content: `${displayName} is ${dir} ${fmtPct(quote.changePercent)} today ` +
    ...
    if (metrics !== null) {
        if (metrics.fiftyTwoWeekHigh > 0 && quote.price >= metrics.fiftyTwoWeekHigh) {
            ...
            if (!isDuplicate(state, key)) {
                onSignal({
                    ...
                    name: quote.name,
                    energy: 0.55,
                    content: `${displayName} has hit a new 52-week high at ${fmt(quote.price)} ` +
        ...
        else if (metrics.fiftyTwoWeekLow > 0 &&
            ...
            if (!isDuplicate(state, key)) {
                onSignal({
                    ...
                    name: quote.name,
                    energy: 0.55,
                    content: `${displayName} has hit a new 52-week low at ${fmt(quote.price)} ` +
        ...
        // ── Unusual volume ────────────────────────────────────────────────────────
        if (metrics.averageVolume10Day > 0 &&
            quote.volume > 0 &&
            ...
            if (!isDuplicate(state, key)) {
                ...
                onSignal({
                    ...
                    name: quote.name,
                    energy: 0.45,
                    content: `${displayName} is seeing unusual trading volume: ${ratio}× its 10-day ` +
 ...
 */
function evaluateIndex(item, quote, state, onSignal) {
...
export class MarketSync {
     ...
     */
    startSync(state, client, onSignal, onSummary) {
        let stopped = false;
        let timer = null;
        /**
            ...
            try {
                ...
                for (const item of state.watchlist) {
                    try {
                        ...
                        console.error(`[market-sync] Failed to fetch quote for ${item.symbol}:`, err);
                    }
                }
                ...
                const summaryDue = isAfterClose && state.lastMarketSummaryDate !== todayStr;
                const isTransitionFromOpen = prevMarketState === "open" && newMarketState !== "open";
...
//# sourceMappingURL=market-sync.js.map