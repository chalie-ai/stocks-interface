 ...
 */
function resolveBadge(marketState) {
    if (marketState === "open") {
 ...
 */
function buildTableRow(row, isEven) {
 ...
 * ```
 */
export async function handleStockCompare(params, client, state) {
    ...
    const rows = symbols.map((symbol, i) => {
        ...
        const quote = quoteResult !== undefined && quoteResult.status === "fulfilled"
            ? quoteResult.value
            : null;
        const quoteError = quoteResult !== undefined && quoteResult.status === "rejected"
            ? (quoteResult.reason instanceof Error
                ? quoteResult.reason.message
                : String(quoteResult.reason))
            : null;
        const metrics = metricsResult !== undefined && metricsResult.status === "fulfilled"
            ? metricsResult.value
            : null;
    ...
    const textParts = successRows.map((r) => {
        ...
        return (`${q.symbol} ${formatPrice(q.price)} ` +
            `${formatChangePercent(q.changePercent)} ` +
            `P/E ${formatPE(pe)}`);
    });
    const text = `Comparison (${symbols.length} stock${symbols.length !== 1 ? "s" : ""}): ` +
        (textParts.length > 0
            ? textParts.join(" | ")
            : "No data available.") +
...
//# sourceMappingURL=stock-compare.js.map