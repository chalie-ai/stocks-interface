 ...
 */
function renderNewsItem(article, isLast) {
 ...
 * ```
 */
export async function handleStockNews(params, client, _state) {
...
<div style="font-family:system-ui,sans-serif;background:#fff;border:1px solid #e5e7eb;
  ...
  <p style="margin:0;color:#6b7280;font-size:14px">${escapeHtml(msg)}</p>
    ...
    // ── Plain-text summary ────────────────────────────────────────────────────
    const textParts = items.map((a) => `"${a.headline}" (${a.source}, ${formatDate(a.datetime)})`);
    ...
    const newsItemsHtml = items
        ...
        .join("\n");
    const articleCountLabel = `${items.length} article${items.length !== 1 ? "s" : ""}`;
    const html = `
<div style="font-family:system-ui,sans-serif;background:#fff;border:1px solid #e5e7eb;
    border-radius:8px;padding:16px;max-width:520px;
