 ...
 */
function dateAtEtTime(etDateStr, hour, minute) {
    ...
    return new Date(`${etDateStr}T${pad2(hour)}:${pad2(minute)}:00-05:00`);
}
/**
 ...
 */
function nextBusinessDateStr(etDateStr, etWeekday) {
 ...
 */
function classifyMarketPhase(isOpen, etHour, etMinute) {
    if (isOpen)
        return "open";
 ...
 */
function computeNextEvent(phase, etParts) {
    const etDateStr = `${etParts.year}-${pad2(etParts.month)}-${pad2(etParts.day)}`;
    if (phase === "open") {
        const closeDate = dateAtEtTime(etDateStr, MARKET_CLOSE_HOUR_ET, MARKET_CLOSE_MINUTE_ET);
        return { label: "Market closes", date: closeDate };
    }
    if (phase === "pre") {
        const openDate = dateAtEtTime(etDateStr, MARKET_OPEN_HOUR_ET, MARKET_OPEN_MINUTE_ET);
 ...
 * ```
 */
export async function handleMarketStatus(_params, client, _state) {
...
<div style="font-family:system-ui,sans-serif;background:#fff;border:1px solid #fecaca;
  ...
  <p style="margin:0;color:#dc2626;font-size:14px">
    ...
    const quoteResults = await Promise.allSettled(INDEX_SYMBOLS.map((sym) => client.quote(sym)));
    /** Resolved quote or null on failure, indexed by position in INDEX_SYMBOLS. */
    const quotes = quoteResults.map((r) => r.status === "fulfilled" ? r.value : null);
 ...
 * ```
 */
export async function handleEarningsCalendar(params, client, _state) {
    // ── 1. Resolve parameters ─────────────────────────────────────────────────
    const symbolFilter = typeof params.symbol === "string" && params.symbol.trim().length > 0
    ...
    const toDate = new Date(Date.UTC(etParts.year, etParts.month - 1, etParts.day + daysAhead));
...
<div style="font-family:system-ui,sans-serif;background:#fff;border:1px solid #e5e7eb;
  ...
  <p style="margin:0;font-size:13px;color:#6b7280">
    ...
    const rows = sorted
        ...
        const epsEstStr = e.epsEstimate !== null ? `$${e.epsEstimate.toFixed(2)}` : "N/A";
        const epsActStr = e.epsActual !== null ? `$${e.epsActual.toFixed(2)}` : "—";
        return `
      <tr style="border-top:1px solid #f3f4f6">
 ...
 */
function reportTimeLabel(reportTime) {
    switch (reportTime) {
        case "before-open":
            return "Before Open";
        case "after-close":
 ...
 * @returns CSS hex colour string.
 */
function reportTimeColor(reportTime) {
    switch (reportTime) {
        case "before-open":
        case "after-close":
            return "#92400e";
