 ...
 */
import { FinnhubAuthError, FinnhubNetworkError, } from "../finnhub/client.js";
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
/**
 ...
 */
const INDEX_ETF_PROXIES = new Set(["SPY", "QQQ", "DIA"]);
// ---------------------------------------------------------------------------
// Exported handler
// ---------------------------------------------------------------------------
/**
 ...
 * ```
 */
export async function handleWatchlistAdd(params, client, state) {
    ...
    try {
        ...
        if (quote.price === 0 && quote.timestamp === 0) {
            return buildErrorResult(`Symbol ${symbol} not found on Finnhub. Please check the ticker and try again.`, state);
        }
    }
    catch (err) {
        if (err instanceof FinnhubAuthError) {
            return buildErrorResult("This API key doesn't seem to work. Please double-check you copied the full key.", state);
        }
        if (err instanceof FinnhubNetworkError) {
            return buildErrorResult("Couldn't reach Finnhub to verify symbol. Please check your internet connection.", state);
        }
        // FinnhubApiError (500-range, rate-limit, etc.) or unexpected error.
        const msg = err instanceof Error ? err.message : String(err);
        return buildErrorResult(`Finnhub seems to be having issues right now (${msg}). Please try again shortly.`, state);
    }
    // ── Step 2: Reject duplicates (case-insensitive) ─────────────────────────
    const alreadyExists = state.watchlist.some((item) => item.symbol.toUpperCase() === symbol);
    if (alreadyExists) {
        return buildErrorResult(`${symbol} is already in your watchlist.`, state);
    }
    // ── Step 3: Enforce maxWatchlistSize ─────────────────────────────────────
    const maxSize = state.settings.maxWatchlistSize;
    if (state.watchlist.length >= maxSize) {
        return buildErrorResult(`Your watchlist is full (${maxSize} symbols maximum). ` +
            "Remove a symbol before adding a new one.", state);
    }
    ...
    const itemType = specifiedType ?? (isIndex ? "etf" : "stock");
 ...
 */
function buildSuccessHtml(item, newSize, maxSize) {
...
//# sourceMappingURL=watchlist-add.js.map