...
function periodToRange(period) {
    ...
    switch (period) {
        ...
        case "ytd": {
            ...
            const jan1Seconds = Math.floor(new Date(`${year}-01-01T00:00:00Z`).getTime() / 1_000);
 ...
 */
function renderSparklineSvg(closes, positiveReturn) {
    if (closes.length === 0) {
    ...
    const points = closes
        ...
        const y = SVG_HEIGHT - SVG_PADDING - ((price - minPrice) / priceRange) * innerH;
 ...
 * ```
 */
export async function handleStockHistory(params, client, _state) {
