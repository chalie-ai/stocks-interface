 ...
 * ```
 */
export function handleWatchlistRemove(params, state) {
    const symbol = params.symbol.toUpperCase().trim();
    // ── Find the target item (case-insensitive) ───────────────────────────────
    const itemIndex = state.watchlist.findIndex((item) => item.symbol.toUpperCase() === symbol);
    if (itemIndex === -1) {
...
<div style="font-family:system-ui,sans-serif;background:#fff;border:1px solid #fecaca;
  ...
  <p style="margin:0;color:#dc2626;font-size:14px">
    ...
    const text = `Removed ${removedItem.symbol} (${removedItem.name}) from your watchlist. ` +
 ...
 */
function buildSuccessHtml(symbol, name, remainingCount) {
...
//# sourceMappingURL=watchlist-remove.js.map