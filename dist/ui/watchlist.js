 ...
 */
function renderQuoteCard(item, quote) {
 ...
 */
function renderSkeletonCard(item) {
    const border = item.isIndex ? CARD_BORDER_INDEX : CARD_BORDER_STOCK;
    const bg = item.isIndex ? CARD_BG_INDEX : "#fff";
    return `<div style="
    background: ${bg};
 ...
 */
export function renderWatchlistSection(items, quotes) {
    if (items.length === 0) {
 ...
 */
export function renderEmptyWatchlist() {
    return `<div style="
    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
    padding: 32px 24px;
    border-radius: 12px;
