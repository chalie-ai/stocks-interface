 ...
 */
import { renderWatchlistSection, renderEmptyWatchlist, } from "./watchlist.js";
// ---------------------------------------------------------------------------
 ...
 */
function renderIndexPill(symbol, displayName, quote) {
    if (quote === null) {
 ...
 *          are no index items.
 */
function renderIndexSummaryRow(state, quotes) {
 ...
 *          are active.
 */
function renderAlertsCountBadge(state) {
 ...
 */
function renderLoadingView(state) {
 ...
 * @returns A non-empty HTML fragment string.
 */
function renderErrorView(state) {
    const lastUpdated = state.lastSyncAt
        ...
        : "never";
    const retryMinutes = Math.round(state.settings.syncIntervalMarketClosed / 60_000);
    const retryLabel = retryMinutes === 1 ? "1 minute" : `${retryMinutes} minutes`;
    return `<div style="
    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
  ">
 ...
 * @returns A non-empty HTML fragment string.
 */
function renderReadyView(state, quotes) {
 ...
 */
export function renderMainView(state, quotes, viewState) {
    switch (viewState) {
        case "loading":
