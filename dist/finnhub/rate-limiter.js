...
export class RateLimiter {
    ...
    drain() {
        ...
        while (this.queue.length > 0 && this.requestsThisMinute < RateLimiter.CEILING) {
            ...
            if (idx === -1)
                break; // All remaining items are blocked by stagger.
    ...
    scheduleDrain(delayMs = 0) {
        ...
        if (this.queue.length > 0) {
            const hasHighPriority = this.queue.some((item) => item.priority < RateLimiter.LOW_PRIORITY_MIN);
            if (!hasHighPriority) {
                const staggerWait = Math.max(0, this.lastLowPriorityDispatch + RateLimiter.STAGGER_MS - now);
