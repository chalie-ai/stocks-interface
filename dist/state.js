...
export async function loadState(dataDir) {
    ...
    try {
        ...
        console.error(`[stocks-interface] Failed to read state file at "${filePath}":`, err);
        return structuredClone(DEFAULT_STATE);
    }
    let parsed;
    try {
        ...
        console.error(`[stocks-interface] state.json at "${filePath}" is not valid JSON; ` +
            "resetting to defaults.");
        return structuredClone(DEFAULT_STATE);
    }
    if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
        console.error(`[stocks-interface] state.json at "${filePath}" contains an unexpected ` +
